    ...
    <details class="evidence-accordion" data-document-id="${escapeHtml(document.document_id)}" ${state.expandedDocuments.has(document.document_id) ? "open" : ""}>
      ...
      <article class="document-card">
        ...
        const sourceDetails = field.page ? `p. ${field.page}<br>` : "No precise source box recovered<br>";
          ...
          const sourceBoxMetadata = field.bbox ? ` · source box [${field.bbox.join(", ")}]` : "";
          return `<tr id="evidence-row-${escapeHtml(key)}" class="${state.highlightedEvidenceKey === key ? "evidence-highlighted" : ""}"><td><label for="evidence-${escapeHtml(key)}"><strong>${escapeHtml(field.field.replaceAll("_", " "))}</strong>${field.extraction_method === "ocr" ? `<span class="ocr-candidate">OCR candidate</span>` : ""}</label><input id="evidence-${escapeHtml(key)}" data-evidence-document="${escapeHtml(document.document_id)}" data-evidence-field="${escapeHtml(field.field)}" value="${escapeHtml(value)}" aria-describedby="evidence-meta-${escapeHtml(key)}"><span id="evidence-meta-${escapeHtml(key)}" class="field-meta">${escapeHtml(ocrMetadata + field.purpose + sourceBoxMetadata)} · ${escapeHtml(confirmation)}</span></td><td>${sourceDetails}${field.bbox ? `<button class="source-locate" type="button" data-highlight-document="${escapeHtml(document.document_id)}" data-highlight-field="${escapeHtml(field.field)}">Highlight</button><br>` : ""}<span class="status ${field.confidence === "high" ? "ready" : "pending"}">${escapeHtml(field.confidence)}</span></td></tr>`;
...
function renderExtractionBenchmark() {
  ...
  $("#extraction-benchmark-content").innerHTML = `<div class="benchmark-metrics"><p><strong>${benchmark.native_text.exact_matches}/${benchmark.native_text.extracted}</strong><span>native-text fixture fields (${benchmark.native_text.exact_match_percent_when_extracted}% exactness)</span></p><p><strong>${benchmark.ocr.exact_matches}/${benchmark.ocr.extracted}</strong><span>strict local OCR candidates (${benchmark.ocr.exact_match_percent_when_extracted}% exactness)</span></p><p><strong>${documents.abstained}</strong><span>fixtures abstained rather than guessed</span></p></div><p class="help-text">${escapeHtml(benchmark.scope)} Native text and local OCR results are reported separately: ${documents.native_text} native-text documents and ${documents.ocr_attempted} local-OCR attempts. ${escapeHtml(benchmark.abstention_policy)}</p>${sourceRecovery}`;
}
