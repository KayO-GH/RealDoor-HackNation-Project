...
function renderDocuments() {
  ...
  $("#document-list").innerHTML = state.payload.documents.map((document) => `
    <details class="evidence-accordion" data-document-id="${escapeHtml(document.document_id)}" ${state.expandedDocuments.has(document.document_id) ? "open" : ""}>
      <summary>
        <span class="document-summary-title"><span class="eyebrow">${escapeHtml(document.document_type.replaceAll("_", " "))}</span><strong>${escapeHtml(document.file_name)}</strong></span>
        <span class="document-summary-metrics"><span>${document.fields.length} allowlisted fields</span><span>${document.page_count} page${document.page_count === 1 ? "" : "s"}</span><span class="status ${document.fields.every((field) => field.confidence === "high") ? "ready" : "pending"}">${document.fields.every((field) => field.confidence === "high") ? "high confidence" : "medium confidence"}</span></span>
      </summary>
      ...
      <div class="document-review-workspace"><div class="document-confirmations"><h5>Confirm values</h5><p class="help-text">Edit a transcription if needed, then confirm the current inputs here.</p><table class="field-table"><thead><tr><th>Allowlisted field</th><th>Source</th></tr></thead><tbody>${document.fields.map((field) => {
        ...
        return `<tr id="evidence-row-${escapeHtml(key)}" class="${highlighted ? "evidence-highlighted" : ""}"><td><label for="evidence-${escapeHtml(key)}"><strong>${escapeHtml(field.field.replaceAll("_", " "))}</strong></label><input id="evidence-${escapeHtml(key)}" data-evidence-document="${escapeHtml(document.document_id)}" data-evidence-field="${escapeHtml(field.field)}" value="${escapeHtml(value)}" aria-describedby="evidence-meta-${escapeHtml(key)}"><span id="evidence-meta-${escapeHtml(key)}" class="field-meta">${escapeHtml(field.purpose)} · ${escapeHtml(confirmation)}</span></td><td><button class="source-locate" type="button" data-highlight-document="${escapeHtml(document.document_id)}" data-highlight-field="${escapeHtml(field.field)}">Highlight</button><br><span class="status ${field.confidence === "high" ? "ready" : "pending"}">${escapeHtml(field.confidence)}</span></td></tr>`;
